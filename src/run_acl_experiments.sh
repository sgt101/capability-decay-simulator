#!/usr/bin/env bash
# Runs every data/experiments-acl/acl.N.json through src/batch_run.js and
# writes each one's three CSVs into results/acl.N/.
#
# The ACL SCENARIO set — the focus results, ~5,400 runs / ~6 CPU-hours. Small enough to
# run in one sitting, unlike the two sweep sets.
#
...
#
# Usage:
#   ./run_acl_experiments.sh                    # all acl.*.json
#   ./run_acl_experiments.sh 1 3 9              # just those numbers
#   ./run_acl_experiments.sh --workers 14       # all, with 14 worker threads
#   ./run_acl_experiments.sh --redo             # re-run even completed ones
#   ./run_acl_experiments.sh --parallel 3 --workers 6
...
if [ ! -d "$CFG_DIR" ]; then
  echo "no $CFG_DIR — generate it first: node src/generate_acl_experiments.js" >&2
  exit 1
fi
...
if [ "$#" -gt 0 ]; then
  ...
  if [ "${#NUMS[@]}" -eq 0 ]; then
    echo "no ${STEM}.*.json in $CFG_DIR — generate it first: node src/generate_acl_experiments.js" >&2
    exit 1
  fi
