// Builds doc/results_ACL.html — the same heatmap browser as report.html, over the
// ACL SCENARIO set — the focus results for the paper.
//
//   node src/build_structure_report.js
//   node src/build_structure_report.js --metrics capability --out results_ACL_capability.html
//
// Every metric in one dropdown, and the grids are lambda x gamma_below with the
// capability variant and gamma_above named per experiment — see
// generate_acl_experiments.js for why the set is arranged that way.
//
...
const STEM = "acl";
const DEFAULT_OUT = "results_ACL.html";
...
if (missing.length === manifest.experiments.length) {
  console.error(`[build_structure_report] no results under results/${STEM}.*/ — run them first:\n`
    + `  ./src/run_acl_experiments.sh --workers 16\n\n`
